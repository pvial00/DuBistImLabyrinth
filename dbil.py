...
def dbil_demo(size):
    print("Generating Alice and Bob's keys"0
    ...
    p6B = pow(p5, skB, U)
    print(p6, p6B)
    print("Generate the secret ephemeral key E")
    EkA = number.getRandomRange(1, (p6 - 1))
    EkB = number.getRandomRange(1, (p6B - 1))
    print(EkA, EkB)
    print("Exchange phase 7")
    p7 = pow(yB, EkA, p6)
    p7B = pow(yB, EkB, p6B)
    print(p7, p7B)
    print("Arrive at the umbrella cloak")
    p8 = pow(p7B, EkA, p6)
    p8B = pow(p7, EkB, p6B)
    print(p8, p8B)
    print("Create the private umbrella")
    U = U * p8
    print(U)
    y = number.getRandomRange(1, (U - 1))
    print("Pick a point in the new U exchange")
    p9 = pow(y, skA, U)
    p9B = pow(y, skB, U)
    print(p9, p9B)
    print("Arrive at the shared key phase10")
    p10 = pow(p9B, skA, U)
    p10B = pow(p9, skB, U)
    print(p10, p10B)
